...
<div class="container">
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()){;
                    ...
                    if ($result_do_an->num_rows > 0) {
                        while($row_do_an = $result_do_an->fetch_assoc()){;
                            $do_an_id = $row_do_an['do_an_id'];
                                $sql_do_an = "SELECT * FROM do_an WHERE do_an_id = '$do_an_id'";                
                        }
                    } 
                }
    ...
    <h1 class="caption">Chọn đồ ăn muốn đổi</h1>
    <form action="XoaDatMonAn.php" method="get">
        <table>
            <tr>
                <th>STT</th>
                <th>Đồ Ăn</th>
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<td>$mo_ta</td>";
                    echo "<td class='td_dat_do_an'><a class='add_button' href='/CuaHangDoAn/DoiDoAn.php?do_an_id=$do_an_id & khach_hang_id=$khach_hang_id' onclick=\"return DoiDoAn('$ten_do_an_cu','$ten_do_an')\">Đổi</a></td>";               
                    echo "</tr>";
                    $stt++; 
                }   
            } 
